...
class Rest
{
	...
	*/
    ...
    protected function _call($method, $uri, $params = array(), $format = NULL)
    {
        ...
        if(!$response AND isset($this->curl->error_string) AND isset($this->curl->error_code))
        {
            error_log("CURL Fatal error({$this->rest_server}{$uri}): ".$this->curl->error_string." (".$this->curl->error_code.") in ".__FILE__." on ".__LINE__, 0);
        }
