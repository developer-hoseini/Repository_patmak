...
class ApplicationRepository implements ApplicationRepositoryInterface{
    ...
    {
        return DB::table('tbl_application AS app')
            ->select('app.application_id' ,'app.created_at', 'req.req_type_title', 'lic.license_type_title', 'org.morg_title', 'pro.province_title', 'edu.study_field_title', 'app.status_id', 'stat.status_title','app.read_user as read')
            ->leftJoin('ref_request_type AS req', 'app.app_type_id', '=', 'req.req_type_id')
            ->leftJoin('ref_license_type AS lic', 'app.license_type_id', '=', 'lic.license_type_id')
            ->leftJoin('ref_mrud_organizations AS org', 'app.mrud_org_id', '=', 'org.morg_id')
            ->leftJoin('ref_provinces AS pro', 'app.province_id', '=', 'pro.province_id')
    ...
    public function getApplications($length = 10, $start = 0, $conditions = null)
    {
        ...
        if(is_array($conditions)){
            if(isset($conditions['application_id']) && ! is_null($conditions['application_id'])){
                $db->where('app.application_id', $conditions['application_id']);
            }
            if(isset($conditions['person_type_title']) && ! is_null($conditions['person_type_title'])){
                $db->where('pt.person_type_title', $conditions['person_type_title']);
            }
            if(isset($conditions['p_name']) && ! is_null($conditions['p_name'])){
                $db->where('pers.p_name', $conditions['p_name'])->orWhere('pers.p_name', 'like','%'.$conditions['p_name'].'%');
            }
            if(isset($conditions['p_lname']) && ! is_null($conditions['p_lname'])){
                $db->where('pers.p_lname', $conditions['p_lname'])->orWhere('pers.p_lname', 'like','%'.$conditions['p_lname'].'%');
            }
            ...
            if(isset($conditions['province_title']) && ! is_null($conditions['province_title']) ){
                $db->where('pro.province_title', $conditions['province_title']);
            }
            if(isset($conditions['status_title']) && ! is_null($conditions['status_title']) ){
                $db->where('stat.status_title', $conditions['status_title']);
            }
            if(isset($conditions['tracking_code']) && ! is_null($conditions['tracking_code']) ){
                $db->where('app.tracking_code', $conditions['tracking_code']);
            }
        }

        $count = $db->orderBy('app.application_id', 'DESC')->get()->count();
        $items = $db->orderBy('app.application_id', 'DESC')->skip($start)->take($length)->get();
